...
const UserChip = ({highLightedUser}) => {
  const {selectedUsers,setSelectedUsers,setUsersList}=useUserData()
    let removeUser=(id)=>{                                   
     let remainingUsers=selectedUsers?.filter((user)=>{       //remove user
           return user.id!==id
     })
  ...
  return (
    <div className='flex gap-2 max-w-max flex-wrap  '>
      {selectedUsers?.map((user)=>{
        ...
        return  <div key={user.id} className={`flex bg-[#ccc] border-2 border-transparent border-solid gap-2 rounded-md py-1 px-[4px] max-w-max text-nowrap ${highLightedUser===user.id && 'border-red-500'} }`}>
